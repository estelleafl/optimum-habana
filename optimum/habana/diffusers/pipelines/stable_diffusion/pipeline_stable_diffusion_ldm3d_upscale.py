...
from .pipeline_stable_diffusion_ldm3d import GaudiStableDiffusionLDM3DPipelineOutput

logger = logging.get_logger(__name__)



class GaudiStableDiffusionUpscaleLDM3DPipeline(
    ...
    def prepare_latents(self, num_images, num_channels_latents, height, width, dtype, device, generator, latents=None):
        shape = (num_images, num_channels_latents,  height, width)
    ...
    def __call__(
        ...
        Args:
            ...
            rgb (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                `Image` or tensor representing an rgb batch to be upscaled.
            depth (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                `Image` or tensor representing an depth batch to be upscaled.
        ...
        with torch.autocast(device_type="hpu", dtype=torch.bfloat16, enabled=self.gaudi_config.use_torch_autocast):
            ...
            self.check_inputs(
                prompt, rgb, noise_level, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds
            )
